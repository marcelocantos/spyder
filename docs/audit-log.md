...
## 2026-04-27 — /release v0.25.0

- **Commit**: `7ff241d`
- **Outcome**: Released v0.25.0. Closes a gap in the v0.23.0 user-opt-out semantics: a Running → backgrounded transition observed by autoawake's KeepAwake state probe was being read as opt-out regardless of cause, but spyder's own `launch_app` and `deploy_app` handlers cause exactly that transition every time they foreground a non-KeepAwake bundle. The result was that an orchestration call (running a test app, deploying a bundle, anything that uses `mcp__spyder__launch_app`) silently disabled autoawake on the device until the user manually re-foregrounded KeepAwake. The fix plumbs a per-device `spyderLaunchedAt` timestamp on the autoawake supervisor's `deviceObs` record. New `Supervisor.NoteAppLaunched(udid, bundleID)` is called by `handleLaunchApp` and `handleDeployApp` (post-success, only for non-KeepAwake bundles) via a new `AutoawakeNotifier` interface in the mcp package — interface kept narrow and defined mcp-side so the package doesn't import autoawake. The `recordKAState` Running → backgrounded edge consults `time.Since(obs.spyderLaunchedAt) < recentLaunchWindow` (30 s) and skips the opt-out flag-set when the marker is fresh, consuming the marker so a subsequent unrelated swipe is interpreted correctly. Plumbing: `mcp.Handler.SetAutoawakeNotifier(a)` sibling to the existing `WithAutoawakeNotifier` option; `daemon.Build` returns the `*spydermcp.Handler` as a fourth value so `Run` can bind the autoawake supervisor (constructed only after `bridgeSup.Start` populates the bridge URL) post-construction. 4 new unit tests cover suppress-on-fresh-marker, allow-opt-out-on-stale-marker, KeepAwake-bundle-is-no-op, and unknown-device-is-no-op. STABILITY.md keep-awake gap entry refreshed; catalogue snapshot bumped to v0.25.0. No public MCP / CLI / REST surface changes — `NoteAppLaunched` is internal plumbing. Published for darwin-arm64, linux-amd64, linux-arm64.
...
## 2026-07-19 — /release v0.71.0

- **Commit**: (filled at tag)
- **Outcome**: Released v0.71.0. 🎯T108 host Starlark durable scripts:
  `list_scripts` / `run_script` verbs + CLI, `app_exec(script_path, params)`,
  pure assert/L1 helpers (`assert_trajectory`, `assert_drag_follow`,
  `assert_settle`, `resolve_target`, `find_by_label`), bundled recipes under
