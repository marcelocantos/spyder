...
| `perf_fps` | `{device, package\|bundle_id, window_sec?, owner?}`. Android only. | JSON `{device, result:{package,window_sec,total_frames,janky_frames,fps,source}, note}`. | Stable (🎯T111) — gfxinfo window; not SurfaceFlinger timestats |
| `port_forward_start` | `{device, device_port, local_port?, owner?}`. Android only. | JSON `{device, local_port, device_port, spec, host_url}`. | Stable (🎯T111) |
| `port_forward_stop` | `{device, local_port, owner?}`. | JSON `{device, local_port, removed}`. | Stable (🎯T111) |
| `port_forward_list` | `{device, owner?}`. | JSON `{device, forwards:[{local_port,device_port,spec},…]}`. | Stable (🎯T111) |
| `input_tap` | `{device, x, y, owner?}`. Android only. | JSON `{device, x, y, injected}`. | Stable (🎯T111) — minimal OS inject, not UI automation |
| `input_swipe` | `{device, x1, y1, x2, y2, duration_ms?, owner?}`. Android only. | JSON `{device, x1, y1, x2, y2, duration_ms, injected}`. | Stable (🎯T111) |
