...
func (c *smokeClient) serve() {
	for {
		...
		case appchannel.MethodSensorSuppress:
			result = map[string]any{"sensor": "accel", "suppressed": true}
		case appchannel.MethodSensorSet:
			result = map[string]any{"sensor": "accel", "suppressed": true, "value": []any{-1.5, 0.0, 0.0}}
		case appchannel.MethodSensorUnsuppress:
			result = map[string]any{"sensor": "accel", "suppressed": false}
...
func TestAppChannel_FullMethodSweep(t *testing.T) {
	...
	client := dialSmoke(t, port, []string{
		...
		appchannel.MethodSensorSuppress, appchannel.MethodSensorSet,
		appchannel.MethodSensorUnsuppress, appchannel.MethodSensorStatus,
	...
	calls := []call{
		...
		{"app_sensor_suppress", map[string]any{"session_id": sessionID, "sensor": "accel"}, func(t *testing.T, body string) {
			if !strings.Contains(body, "suppressed") {
				t.Errorf("suppress body: %s", body)
			}
		}},
		{"app_sensor_set", map[string]any{"session_id": sessionID, "sensor": "accel", "value": []any{-1.5, 0.0, 0.0}}, func(t *testing.T, body string) {
			if !strings.Contains(body, "-1.5") && !strings.Contains(body, "value") {
				t.Errorf("set body: %s", body)
			}
		}},
		{"app_sensor_unsuppress", map[string]any{"session_id": sessionID, "sensor": "accel"}, func(t *testing.T, body string) {
			if !strings.Contains(body, "suppressed") {
				t.Errorf("unsuppress body: %s", body)
			}
		}},
