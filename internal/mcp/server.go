...
func allBaseDefinitions() []mcpgo.Tool {
	return []mcpgo.Tool{
		...
		mcpgo.NewTool("perf_fps",
			mcpgo.WithDescription("Measure Android UI/compositor frame rate over an N-second window via dumpsys gfxinfo (reset → wait → dump). Returns total_frames, janky_frames, and fps = total/window_sec. Android only. For cooperative ge push gauges use app_perf_get; for full frame rings use app_metrics_*. Do not shell out to adb for this."),
		...
		mcpgo.NewTool("port_forward_start",
			mcpgo.WithDescription("Tunnel a device TCP port to the host (adb forward parity). Android only. local_port 0 = ephemeral. Returns local_port and host_url. Pair with port_forward_stop / port_forward_list. Do not shell out to adb forward."),
			mcpgo.WithString("device", mcpgo.Required(), mcpgo.Description("Android device alias or serial")),
			mcpgo.WithNumber("device_port", mcpgo.Required(), mcpgo.Description("TCP port listening on the device")),
			mcpgo.WithNumber("local_port", mcpgo.Description("Host local port (0 = let adb choose)")),
			mcpgo.WithString("owner", mcpgo.Description("Reservation owner when the device is reserved")),
		),
		mcpgo.NewTool("port_forward_stop",
			mcpgo.WithDescription("Remove a host→device TCP forward by local_port (adb forward --remove). Android only."),
			mcpgo.WithString("device", mcpgo.Required(), mcpgo.Description("Android device alias or serial")),
			mcpgo.WithNumber("local_port", mcpgo.Required(), mcpgo.Description("Host local port to remove")),
			mcpgo.WithString("owner", mcpgo.Description("Reservation owner when the device is reserved")),
		),
		mcpgo.NewTool("port_forward_list",
			mcpgo.WithDescription("List active TCP forwards for this Android device (adb forward --list filtered by serial)."),
			mcpgo.WithString("device", mcpgo.Required(), mcpgo.Description("Android device alias or serial")),
			mcpgo.WithString("owner", mcpgo.Description("Reservation owner when the device is reserved")),
		),
		mcpgo.NewTool("input_tap",
			mcpgo.WithDescription("OS-level tap at pixel coordinates on Android (adb shell input tap). Minimal inject for scripted perf runs — not UI automation (no tree/OCR; use mobile-mcp for that). Prefer app_input for ge apps with app-channel."),
			mcpgo.WithString("device", mcpgo.Required(), mcpgo.Description("Android device alias or serial")),
		...
		mcpgo.NewTool("input_swipe",
			mcpgo.WithDescription("OS-level swipe on Android (adb shell input swipe). Minimal inject for scripted runs — not full UI automation. Prefer app_input for cooperative ge apps."),
			mcpgo.WithString("device", mcpgo.Required(), mcpgo.Description("Android device alias or serial")),
