...
func (h *Handler) toolHandlers() map[string]toolFunc {
	return map[string]toolFunc{
		...
		"device_setting":     h.handleDeviceSetting,
...
func allBaseDefinitions() []mcpgo.Tool {
	return []mcpgo.Tool{
		...
		// ---- Android OS control (🎯T111) — no direct adb for callers ------

		mcpgo.NewTool("perf_fps",
			mcpgo.WithDescription("Capability: FPS over an N-second window. Android: dumpsys gfxinfo (real). iOS: fails closed — use app_perf_get / app_metrics_* for cooperative ge counters (no free compositor FPS). Do not shell out to adb."),
			mcpgo.WithString("device", mcpgo.Required(), mcpgo.Description("Device alias or platform id (android or ios)")),
			mcpgo.WithString("package", mcpgo.Description("App package / bundle id")),
			mcpgo.WithString("bundle_id", mcpgo.Description("Alias for package")),
			mcpgo.WithNumber("window_sec", mcpgo.Description("Sample window in seconds (default 3, max 120; Android path uses DeadlinePerfFPS=150s)")),
			mcpgo.WithString("owner", mcpgo.Description("Reservation owner when the device is reserved")),
		),
		mcpgo.NewTool("port_forward_start",
			mcpgo.WithDescription("Capability: tunnel a device TCP port to the host. Android: adb forward. iOS: go-ios usbmux forward (iproxy-class). local_port 0 = ephemeral. Do not shell out to adb/iproxy."),
			mcpgo.WithString("device", mcpgo.Required(), mcpgo.Description("Device alias or platform id (android or ios)")),
			mcpgo.WithNumber("device_port", mcpgo.Required(), mcpgo.Description("TCP port listening on the device")),
			mcpgo.WithNumber("local_port", mcpgo.Description("Host local port (0 = ephemeral)")),
			mcpgo.WithString("owner", mcpgo.Description("Reservation owner when the device is reserved")),
		),
		mcpgo.NewTool("port_forward_stop",
			mcpgo.WithDescription("Remove a host→device TCP forward by local_port (Android adb --remove; iOS closes usbmux listener)."),
			mcpgo.WithString("device", mcpgo.Required(), mcpgo.Description("Device alias or platform id")),
			mcpgo.WithNumber("local_port", mcpgo.Required(), mcpgo.Description("Host local port to remove")),
			mcpgo.WithString("owner", mcpgo.Description("Reservation owner when the device is reserved")),
		),
		mcpgo.NewTool("port_forward_list",
			mcpgo.WithDescription("List active TCP forwards for this device (Android adb forward --list; iOS adapter-tracked usbmux forwards)."),
			mcpgo.WithString("device", mcpgo.Required(), mcpgo.Description("Device alias or platform id")),
			mcpgo.WithString("owner", mcpgo.Description("Reservation owner when the device is reserved")),
		),
		mcpgo.NewTool("input_tap",
			mcpgo.WithDescription("Capability: OS-level pixel tap. Android: adb shell input tap. iOS: fails closed (use app_input or mobile-mcp). Not full UI automation."),
			mcpgo.WithString("device", mcpgo.Required(), mcpgo.Description("Device alias or platform id")),
			mcpgo.WithNumber("x", mcpgo.Required(), mcpgo.Description("Pixel X")),
			mcpgo.WithNumber("y", mcpgo.Required(), mcpgo.Description("Pixel Y")),
			mcpgo.WithString("owner", mcpgo.Description("Reservation owner when the device is reserved")),
		),
		mcpgo.NewTool("input_swipe",
			mcpgo.WithDescription("Capability: OS-level pixel swipe. Android: adb shell input swipe. iOS: fails closed (use app_input or mobile-mcp)."),
			mcpgo.WithString("device", mcpgo.Required(), mcpgo.Description("Device alias or platform id")),
			mcpgo.WithNumber("x1", mcpgo.Required(), mcpgo.Description("Start pixel X")),
			mcpgo.WithNumber("y1", mcpgo.Required(), mcpgo.Description("Start pixel Y")),
			mcpgo.WithNumber("x2", mcpgo.Required(), mcpgo.Description("End pixel X")),
			mcpgo.WithNumber("y2", mcpgo.Required(), mcpgo.Description("End pixel Y")),
			mcpgo.WithNumber("duration_ms", mcpgo.Description("Swipe duration in ms (default 300)")),
			mcpgo.WithString("owner", mcpgo.Description("Reservation owner when the device is reserved")),
		),
		mcpgo.NewTool("device_setting",
			mcpgo.WithDescription("Capability: set/restore/get an allowlisted Android system setting (🎯T130). Key refresh_rate is required (writes peak+min). restore=true deletes the pin (does not leave a value). Unknown keys are rejected — no arbitrary adb shell. iOS: same verb, fails closed with 'not supported'."),
			mcpgo.WithString("device", mcpgo.Required(), mcpgo.Description("Device alias or platform id")),
			mcpgo.WithString("key", mcpgo.Required(), mcpgo.Description("Allowlisted key (refresh_rate)")),
			mcpgo.WithString("value", mcpgo.Description("Value to set (e.g. \"60\" or \"120\"). Omit with restore=true or to get.")),
			mcpgo.WithBoolean("restore", mcpgo.Description("Delete the setting (restore OEM default) instead of writing a value.")),
			mcpgo.WithBoolean("get", mcpgo.Description("Read the current values without changing them.")),
			mcpgo.WithString("owner", mcpgo.Description("Reservation owner when the device is reserved")),
		),
