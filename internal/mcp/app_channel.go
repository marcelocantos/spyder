...
func appChannelDefinitions() []mcpgo.Tool {
	return []mcpgo.Tool{
		...
		mcpgo.NewTool("app_state", mcpgo.WithDescription("Query a named slice of the app's state. The app's hello advertises which slices it supports."),
			...
			mcpgo.WithString("select", mcpgo.Description("Optional jq expression evaluated server-side.")),
		),
		mcpgo.NewTool("wait_state",
			mcpgo.WithDescription("Poll a named state slice until a jq select is truthy, then return the satisfying value (🎯T129). Address by session_id or device+bundle_id. On timeout the error includes the last observed value, not a bare 'timed out'. Use select='select(.present and .active)' to get the object back; a boolean expression returns true. Existing assert_* helpers are unchanged."),
			mcpgo.WithString("session_id", mcpgo.Description("Target session id. Alternatively pass device+bundle_id; omit all three when only one session is connected.")),
			mcpgo.WithString("device", mcpgo.Description("Device alias or UUID — used with bundle_id to resolve the keyed listener when session_id is omitted.")),
			mcpgo.WithString("bundle_id", mcpgo.Description("App bundle id — used with device to resolve the keyed listener when session_id is omitted.")),
			mcpgo.WithString("slice", mcpgo.Required(), mcpgo.Description("State slice name to poll (e.g. \"carousel\", \"session\").")),
			mcpgo.WithString("select", mcpgo.Description("jq expression whose truthiness is the predicate. Empty = the whole slice (truthy if non-null). Empty-array jq results keep waiting.")),
			mcpgo.WithNumber("timeout_ms", mcpgo.Description("How long to wait (default 10000, max 120000).")),
			mcpgo.WithNumber("poll_ms", mcpgo.Description("Poll interval (default 200, min 10).")),
		),
