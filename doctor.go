...
func installSudoers() {
	...
	content := sudoersContent(user, resolved)
...
// sudoersContent builds the body of /etc/sudoers.d/spyder-killusbmuxd
// for the given user and absolute helper path. Two directives:
//
//  1. `Defaults!<path> !authenticate` — bypass PAM auth (incl. TouchID)
//     for invocations of this specific command. Without this, sudo's
//     `-n` flag fails in non-tty contexts (e.g. the spyder daemon
//     under launchd) because pam_tid.so wants user interaction even
//     when NOPASSWD is set. With it, sudo skips PAM entirely for
//     this command and NOPASSWD does the rest.
//  2. `<user> ALL=(root) NOPASSWD: <path>` — the actual permission
//     grant. The path must match the daemon's literal invocation.
//
// Pure function — separated from installSudoers so the format is
// testable without touching /etc/.
func sudoersContent(user, resolved string) string {
	return fmt.Sprintf(`# Generated by `+"`"+`spyder doctor --install-sudoers`+"`"+` (🎯T66).
# Grants %s NOPASSWD sudo for the spyder usbmuxd-restart helper.
# The Defaults!…!authenticate line bypasses PAM (incl. TouchID) so the
# spyder daemon's `+"`"+`sudo -n`+"`"+` from a non-tty context succeeds — the
# auto-recovery path in `+"`"+`wedge.RunMonitor`+"`"+` depends on this.
