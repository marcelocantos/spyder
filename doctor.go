...
func runDoctor(args []string) {
	...
	for _, a := range args {
		...
		case "--json":
			jsonOut = true
		case "--help", "-h":
			fmt.Print(`Usage: spyder doctor [--fix] [--json]

Probes the local iOS device stack and reports inconsistencies between
xcrun devicectl's view and go-ios's usbmux view. Without --fix, only
diagnoses. With --fix, runs the bundled spyder-killusbmuxd helper via
sudo (configure NOPASSWD via /etc/sudoers.d/spyder-killusbmuxd for
auth-free recovery).
`)
			return
		default:
